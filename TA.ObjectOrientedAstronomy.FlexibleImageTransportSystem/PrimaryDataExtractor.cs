...
// 
// File: PrimaryDataExtractor.cs  Last modified: 2018-07-11@23:16 by Tim Long

using System;
using System.IO;
using TA.ObjectOrientedAstronomy.FlexibleImageTransportSystem.JonSkeet.MiscUtil;
...
namespace TA.ObjectOrientedAstronomy.FlexibleImageTransportSystem
    ...
    public static class PrimaryDataExtractor
        ...
        public static double[,] ExtractDataArray(FitsHeaderDataUnit hdu)
            ...
            using (var reader = new EndianBinaryReader(EndianBitConverter.Big, inStream))
                {
        ...
        private static Func<EndianBinaryReader, double> GetPixelReader(int bitsPerPixel)
            {
            switch (bitsPerPixel)
                {
                    case -32:
