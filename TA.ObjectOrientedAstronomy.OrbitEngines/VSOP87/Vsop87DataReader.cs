...
// 
// File: Vsop87DataReader.cs  Last modified: 2016-10-08@18:50 by Tim Long
...
namespace TA.ObjectOrientedAstronomy.OrbitEngines.VSOP87
    ...
    internal sealed class Vsop87DataReader
        {
        private const string Vsop87HeaderRegexPattern =
            @"^\s*VSOP87 VERSION (?<VsopVersion>\w+)\s+(?<Body>\w+)\s+VARIABLE (?<Variable>\d+)\s+\((?<Variables>\w+)\)\s+\*T\*\*(?<Power>\d+)\s+(?<Terms>\d+)\s+TERMS\s+(?<Description>.*)";
        private const string Vsop87DataRowRegexPattern =
            @"^(\s*[+-]*[0-9.]+){16}\s+(?<A>[0-9.]+)\s+(?<B>[0-9.]+)\s+(?<C>[0-9.]+).*$";
        internal static readonly Regex Vsop87HeaderRegex = new Regex(Vsop87HeaderRegexPattern, RegexOptions.Compiled);
        private static readonly Regex Vsop87DataRowRegex = new Regex(Vsop87DataRowRegexPattern, RegexOptions.Compiled);
        ...
        public static Vsop87Solution LoadVsop87DataFromFile(string filename)
            ...
            foreach (var variable in solution.CoordinateVariables)
                {
        ...
        ///     Data file was not in the expected format (could not recognize a data
        ///     row or a header row)
        /// </exception>
        private static string LoadPowerSeries(List<Vsop87Term> minorSeries, StreamReader inputStream)
            ...
            while (!inputStream.EndOfStream)
                ...
                throw new InvalidOperationException(
                    "Data file was not in the expected format (could not recognize a data row or a header row)");
                }
    ...
    public sealed class Vsop87Solution
        ...
        public IDictionary<char, IEnumerable<IEnumerable<Vsop87Term>>> CoordinateVariableSeriesData { get; }
        ...
        internal static Vsop87Solution FromHeaderString(string header)
            ...
            Log.Info("Creating VSOP87 solution with Body={0} Version={1} Variables={2} Description={3}",
                ...
                variables,
                description);
            return new Vsop87Solution
                    {Body = body, Version = version, CoordinateVariables = variables, Description = description};
            }
        }
    }