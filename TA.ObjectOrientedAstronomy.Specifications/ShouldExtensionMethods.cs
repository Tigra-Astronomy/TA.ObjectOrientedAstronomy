...
// 
// File: ShouldExtensionMethods.cs  Last modified: 2014-01-31@18:48 by Tim Long
...
namespace TA.Orbits.Specifications
    ...
    public static class ShouldExtensionMethods
        ...
        static readonly Regex EscapeNonFormatBraces = new Regex("{([^\\d].*?)}",
        ...
        public static void ShouldBeLikeObjectGraph(this object obj, object expected, double tolerance)
            ...
            if (exceptions.Any())
                {
                throw NewException(
                    exceptions.Select(e => e.Message).Aggregate("",
                        (r, m) => r + m + Environment.NewLine + Environment.NewLine).TrimEnd());
                }
            }

        static IEnumerable<SpecificationException> ShouldBeLikeObjectGraphInternal(object obj,
            object expected,
            string nodeName,
            double tolerance)
            {
            ...
            if (nodeType == typeof(ObjectGraphHelper.SequenceNode))
                ...
                if (actualNode.GetType() != typeof(ObjectGraphHelper.SequenceNode))
                    {
                    var errorMessage = string.Format("  Expected: Array or Sequence{0}  But was:  {1}",
                        Environment.NewLine,
                        obj.GetType());
                    return new[]
                        {NewException(string.Format("{{0}}:{0}{1}", Environment.NewLine, errorMessage), nodeName)};
                ...
                if (expectedCount != actualCount)
                    {
                    var errorMessage = string.Format("  Expected: Sequence length of {1}{0}  But was:  {2}",
                        Environment.NewLine,
                        expectedCount,
                        actualCount);
                    return new[]
                ...
                return Enumerable.Range(0, expectedCount)
                    .SelectMany(i => ShouldBeLikeObjectGraphInternal(actualValues.ElementAt(i)(),
            ...
            if (nodeType == typeof(ObjectGraphHelper.KeyValueNode))
                ...
                if (actualNode.GetType() != typeof(ObjectGraphHelper.KeyValueNode))
                    {
                    var errorMessage = string.Format("  Expected: Class{0}  But was:  {1}",
                        Environment.NewLine,
                        obj.GetType());
                    return new[]
                        {NewException(string.Format("{{0}}:{0}{1}", Environment.NewLine, errorMessage), nodeName)};
                ...
                return expectedKeyValues
                    .SelectMany(kv =>
                        ...
                        if (actualKeyValue == null)
                            ...
                            return new[]
                                {
                                NewException(string.Format("{{0}}:{0}{1}", Environment.NewLine, errorMessage),
                                    fullNodeName)
                                };
                            }

                        return ShouldBeLikeObjectGraphInternal(actualKeyValue.ValueGetter(),
                            kv.ValueGetter(),
                            fullNodeName,
                            tolerance);
                        });
        ...
        static string Tab(this string str)
            ...
            foreach (string str1 in strArray.Skip(1))
                {
